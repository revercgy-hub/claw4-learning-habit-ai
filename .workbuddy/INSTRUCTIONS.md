...
当前唯一允许执行的工作是修订 `WB-001`。请切换到已有分支 `workbuddy/wb-001-platform-map`，完整读取 `docs/project_management/reports/CODEX_REVIEW_WB-001_2026-09-01.md`，只处理报告列出的四类问题，并只修改：
...
修订两个交付文件、逐项补充验证证据并提交后停止，再次向 Codex 报告 `REVIEW_READY`。不要重写已经通过的章节，不要开始 WB-002。如果需要实机、串口、范围外修改或证据发生无法解释的冲突，生成 `BLOCKED` 报告并停止。
