...
状态 READY，当前唯一 WorkBuddy 活动流。候选08统一包含音频归一化/同步采集、隐藏SSID回退、网络失败恢复。06/07包HOLD且SUPERSEDED，不再执行旧候选刷写命令。先独立代码复核，再按本包核对并测试；Codex负责修正与最终复检。
...
运行62项Python unittest、C++ policy测试与 `test_network_station.py` 16场景（命令见网络失败恢复报告）。使用 `E:/workbuddy/claw4-v6/toolchains/idf61/python_env/idf6.1_py3.12_env/Scripts/python.exe`。组件实码测试以只读 `--source E:/v6/s1` 产生临时Host文件，不重建固件。
...
CP1通过且CP2无阻塞失败时，candidate08执行20轮USB复位（repeat_boot），随后30份60秒空载观察，不模拟硬件故障。记录wall-clock/uptime/采集缺口；180健康样本只是期望，不补造。异常立即停止；USB复位不称冷启动，空载不称语音负载稳定。无联网时不称网络稳定通过。
