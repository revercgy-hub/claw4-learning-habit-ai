# 历史看板快照（SUPERSEDED：仅停止作为当前调度入口）

来源：7dd6511，归档日期 2026-09-13。历史证据不删除、不倒改验收结论；当前状态见 [TASK_BOARD](TASK_BOARD.md)。D2/D5/D8 等未完成项已承接，归档不代表它们通过验收。

---

# 项目任务看板

> **2026-09-06 最新覆盖结论**：Host connected checkpoint `CHECKPOINT_READY`；Full gate PASS（C++ 16/16、Backend 79、PWA 31、HTTP 5 轮/55 次真实进程重启），PWA 三个真实创建任务均验证 2 分钟/暂停 1 次/恰好一次记录。COM7 ota_0 application-only 复测已完成：用户授权并通过“重新生成演示任务”清空 learning NVS、重新 seed，随后屏侧本地状态链路交互正常；当前限定子门禁 `L1_DEMO_RESET=ACCEPTED`。设备 Backend 会话层（认证/今日快照/事件 ACK/诊断）已完成主机测试 17/17 与 C5 BUILD ONLY，状态 `CHECKPOINT_READY`；真实 endpoint/signer、真机网络与刷机验收仍待下一门禁。详见 [L1 reset 报告](reports/CODEX_APP_FIRST_L1_RESET_2026-09-06.md)、[AF3-5 会话报告](reports/CODEX_APP_FIRST_AF3_SESSION_2026-09-06.md)、[connected 报告](reports/CODEX_APP_FIRST_CONNECTED_2026-09-06.md) 与 [硬件复核](reports/CODEX_HW_COM7_READONLY_2026-09-06.md)。
...
| 6.27 | CODEX-APP-FIRST-001 / AF3-5 ota_0 application-only flash | Codex | `CHECKPOINT_READY`（`FLASH_PASS_HASH_VERIFIED`；真实联网待验收） | 6.26；用户明确授权刷机 | COM7 / ESP32-P4 写入 `0x200000`，`Hash of data verified`，RTS 硬复位；只读复位后连通性检查通过；报告 `CODEX_APP_FIRST_AF3_FLASH_2026-09-06.md`。 |
| 6.28 | CODEX-APP-FIRST-002 / 设备 Backend 真机联调 | Codex | **`CHECKPOINT_READY`（阻塞已解除；`BLOCKED` 判定为误判）** | 6.27；AF3-5 刷写校验通过 | 原判定「Windows 入站防火墙拦截」**不成立** —— 防火墙规则（专用 `192.168.3.0/24`、公用 `192.168.3.49/32` → TCP 18765）自始正确且已启用。**真因是本机 Tailscale exit node + `192.168.3.0/24` 子网路由广播劫持局域网回程**：设备 `0x71`(EHOSTUNREACH) 快速失败、主机 `netstat` 无任何连接记录。关闭 Tailscale 后链路立即恢复，`challenge/auth/today/events` 全链 200。详见 6.31 与 `WB-APP-FIRST-L3_NETWORK_ACCEPTANCE_2026-09-12.md` §6.2。 |
| 6.29 | CODEX-APP-FIRST-002 / P1-P2 provisioning + HMAC signer | Codex | `CHECKPOINT_READY`（C5 BUILD ONLY；NVS merged image 已写入并 SHA 回读一致） | 6.28；用户确认 provisioning 方案 | 独立 `learning_cfg` NVS 配置、mbedTLS HMAC-SHA256/Base64 signer、`ConfigureProvisionedBackend()`；本次 C5 worker 候选 `xiaozhi.bin` SHA-256 `682FFBCB…1075C44`；报告 `CODEX_APP_FIRST_002_P1P2_2026-09-06.md` 与 `CODEX_APP_FIRST_002_P3_P6_2026-09-06.md`。 |
| 6.30 | CODEX-APP-FIRST-002 / P3 backend worker + device diagnostics | Codex | `CHECKPOINT_READY`（目标编译通过；仅保留 redacted diagnostics；设备端待进入学习页触发首次 cycle） | 6.29；P1/P2 | LearningRuntime 启动时加载 `learning_cfg`，后台 FreeRTOS worker 每 15s 执行 auth/today/events，状态锁保护 UI/worker；C5 build 通过，既有 ota_1 overflow warning 不变。 |
| 6.31 | WB-APP-FIRST-L3 / 设备 Backend 真机链路验收（L3 NETWORK 收口） | WorkBuddy | **`CHECKPOINT_READY`（P6 六项 6/6 通过；验收决策归用户）** | 6.28 阻塞解除；分支 `workbuddy/app-first-l3-acceptance`（基线 `8bac329`） | **端到端链路 PASS**：设备 `192.168.3.49` → relay `192.168.3.26:18765` → backend `127.0.0.1:8000`。P6 逐项：①屏侧 build/diagnostics ✓ ②在线拉取今日任务 ✓ ③**断网** Start→Pause→Resume→Complete 4 次触控全受理（`status=0 intent=0`）且逐次落 NVS ✓ ④重启保留（`boot with committed state: tasks=1 active=1`）✓ ⑤恢复网络补传 `events 15→20`、`pending 5→0`、`ACK 15→20`、`accepted=20 / duplicates=0 / rejected=0` ✓ ⑥家长侧唯一 `completed` 记录 ✓。**设备固件冻结**（只读回读 `0x200000` 前 8 KiB）：实机 = `claw4-idf-cold-c5-20260906\xiaozhi.bin`（9,255,536 B，app_desc `elf_sha256 7b0944d5…319a58`），与 6.29 记录候选 `682FFBCB…`（9,249,600 B）**不符**，旧记录建议标 `SUPERSEDED`。**新发现**：`pause_count` 未进固件（§5.1 陈旧 `reducer.obj`）、设备时间戳为开机秒（§5.2 SNTP 待接）、异常复位丢 running 段时长（§5.3）。未写 Flash、未改代码、未发生范围偏差。报告 `WB-APP-FIRST-L3_NETWORK_ACCEPTANCE_2026-09-12.md`。 |
| 6.32 | WB-APP-FIRST-L3-BUILD-FIX / 陈旧目标文件缺陷修复 + 固件重建 + 真机端到端验证 | WorkBuddy | **`CHECKPOINT_READY`（修复已经真机验证；验收决策归用户）** | 6.31（`a14f5c4`） | **根因**：镜像同步回拨源文件 mtime（`reducer.cpp` 09:27 < obj 18:44），ninja 判定"最新"跳过重编。**决定性证据**：obj **段归属分析**显示 `pause_count` 仅存在于 `.debug_str`（DWARF 成员名，非字面量），而 `actual_seconds` 在 `.rodata._ZNK…6reduce…str1.4`。**修复**：`touch` `E:/c/main` 385 个源文件 → 增量重建 `rc=0`（**未改任何源码**）。**校验**：`pause_count` 0→1 且位于 payload 正确位置（`task_id/session_id/completion_type/actual_seconds/pause_count`）；`esptool image_info` 仅 rodata +256 B（9,255,536 → **9,255,792 B**），其余段仅对齐位移 → **无其他隐藏差异**；`sdkconfig` md5 `d8874d49…` **逐字节未变**。**新候选** SHA-256 `20a73b70…19cf0`。**真机刷写**（用户 09-13 授权，仅 ota_0 application-only）→ `Hash of data verified`；只读回读首 8 KiB 与本地**逐字节一致**（`56b729f7…`），app_desc `elf_sha256` 由 `7b0944d5…` → `7f3be21d…`，本地/设备三方吻合。**端到端验证 PASS**：Start→Pause→Complete 事件全 accepted（seq 1–5、ack 0→5、rejected=0），`study_sessions.pause_count` **0 → 1**。报告 §11。 |
| 6.33 | 缺陷登记 / `ResetToSeed()` 致序号与后端失配，事件全被拒 | WorkBuddy | **`READY`（待用户决定是否排入修复队列）** | 6.32 | `learning_runtime.cpp:65` 的 `NvsOutboxStorage::eraseAll()` 把持久化的 `next_sequence` 一并擦除 → `outbox_core.cpp:198` 回退为 1；而后端 `last_acked_sequence=20`（`expected=21`）→ `main.py:389` 判 `sequence_regression` → **5 条全拒、ACK 永久卡住、每 15s 重试同样被拒**。触发条件 = 设备端存在历史 ack 的后端 + 设备端执行 `ResetToSeed()`。本次以「备份 DB → 清空 events → ack 归零」**绕过**（非修复，再触发即复现）。建议方案 A（ResetToSeed 后按后端 ack 重新起算）或 B（设备端按 batch 响应自愈）；**方案 C（后端放宽 `seq<=ack`）不推荐**，会破坏幂等。附注：`ResetToSeed()` 还会用 demo 快照**覆盖后端下发的今日任务**。详见报告 §12。 |
...
| 阻塞 ID | 状态 | 解除证据 | 结果 |
| --- | --- | --- | --- |
| BLK-BUILD-STALE-001 | `RESOLVED` | [`WB-APP-FIRST-L3_BUILD_STALE_FIX_2026-09-12.md`](reports/WB-APP-FIRST-L3_BUILD_STALE_FIX_2026-09-12.md) | 根因确认为镜像同步回拨 mtime 致 ninja 漏编 `reducer.cpp`（源 09:27 < obj 18:44）。`touch` `E:/c/main` 385 个源文件后增量重建：`pause_count` 0→1，固件仅 rodata **+256 B**、无其他隐藏差异，`sdkconfig` md5 未变。新候选 `20a73b70…`（9,255,792 B），待用户授权刷写 |
