...
| 6.23 | WB-LEARNING-V4-L1 Learning App 设备基本功能 | WorkBuddy | `IN_PROGRESS`（**L1a ✅ / L1b ✅**：build PASS；**L1c 上机待安排**） | `187b544` | L1a（C30）：codec+NVS+clock+编入（报告 §16）；L1b（C31）：LearningApp id 工厂透传 + `LearningRuntime`（NVS/clock/seed/NVS 计数器）+ 屏真实渲染（UI 零业务状态、Touch dispatch、1s timer 投影、专注秒实时）；host gate **10/10**、`idf.py build` exit=0、bin **9,169,056 B**、sdkconfig diff=0、ota_0 余量 ≈270 KB；报告 §16/§17。L1c：上机验证 seed→Start→暂停→继续→完成→重启恢复（批次 ota_0 app-flash+monitor） |
