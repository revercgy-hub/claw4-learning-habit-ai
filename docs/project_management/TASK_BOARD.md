...
| 6.24 | CODEX-APP-FIRST-001 / L2-L3 MVP 全链批次 | Codex | `IN_PROGRESS`（Host 全链与 AF3 BUILD ONLY `CHECKPOINT_READY`；真机网络/凭据/屏侧诊断验收待后续） | 6.23 `CHECKPOINT_READY` | `bf16b03`：64/64 白名单镜像 SHA 校验、CMake source registration、C5 BUILD ONLY；最新镜像 9,176,176 B，SHA-256 `f3a8a58c…`；报告 `CODEX_APP_FIRST_CONNECTED_2026-09-06.md`。仍禁止刷写，下一门禁为设备注册/凭据与真机网络链。 |
