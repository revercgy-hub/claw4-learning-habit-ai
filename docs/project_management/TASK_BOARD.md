...
| D2 ResetToSeed | 未修：可清pending/sequence并替换正式任务，A02优先 |
| D8网络阻塞 | 未修：主循环同步I/O + runtime共享锁跨网络等待，A03优先 |
| 快照覆盖离线终态 | 源码高风险待定向复现，A04 |
| D5 double-free | 最新源码已修，修复候选真机验证未完成，A05 |
| Time/Reminder | 可信epoch与独立提醒模块尚未实现 |
| L4 Voice | 实验功能链通，Voice Gate未过；NAS方案未验证 |
| 本轮Host | 18/18 suites PASS；41/41头、3/3源、1/1契约语法检查PASS；非设备build |
...
| B03 | Reminder Host与恢复 | Reminder agent | QUEUED | B01/B02接口 + A04合并契约 |
| A05-BUILD | M0候选冻结/已有D5回归 | Integration agent + 主agent | QUEUED | A01–A04；精确patch与工具链可用 |
| A05-DEVICE | M0同一候选上机 | 主agent协调 | HOLD | A05-BUILD + 候选批次授权 |
| C01 | Backend TodayPlan/时间元数据 | Backend agent | QUEUED | A04/B01 schema审查 |
| C02 | PWA计划编辑 | PWA agent | QUEUED | C01契约冻结；最终对真实API集成 |
| C03-HOST/BUILD | Device Time/Reminder薄适配 | Device agent | QUEUED | A01–A04/B01–B03/C01；限定文件与vendor边界 |
| C03-DEVICE | 本地提醒/离线/误差验收 | 主agent协调 | HOLD | M0设备Gate + C03构建 + 批次授权 |
| C04 | Focus/DailyReview/习惯闭环 | Habit agent | QUEUED | M1/M2；共享文件分提交串行 |
| D01 | 语音服务路线合成验证 | Voice/Server agent | QUEUED | B02；先研究/fixture，不自动部署 |
| D02 | Voice/MCP/Level-2联合 | Voice/MCP agent | QUEUED | 本地提醒Gate + D01；真机部分需候选授权 |
| D03 | 家庭试用安全/全链/长稳 | Security/Integration agent | BACKLOG | M1–M4；真实数据/部署另定范围 |
| E01 | ScreenOff/LightSleep/Wake | Power agent | HOLD | M1稳定 + 新板级授权；不阻塞M1–M4 |
| E02 | 个性化/复杂重复规则 | 后续 | BACKLOG | 客观数据可信、核心MVP稳定 |

初始建议派发：A01、A02、B01，各一独立分支，主agent保持审查槽。A03前置线程ADR由主agent控制；不以增加并行数替代状态所有权设计。
