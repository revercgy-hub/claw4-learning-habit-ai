...
| 6.23 | WB-LEARNING-V4-L1 Learning App 设备基本功能 | WorkBuddy | `IN_PROGRESS`（**L1a ✅**：codec/device ports/编入 build PASS；L1b 屏接真实进行中） | `41f3646` | L1a（C30）：`device/core/outbox_codec`（roundtrip+corrupt 负例全绿）+`demo_seed.h`+`device/ports/learning_clock`（esp_timer）+`nvs_outbox_storage`（NVS ns learning）；镜像 `E:/c/main/learning` + manifest #3（INCLUDE_DIRS+SOURCES 10 行）；host gate **10/10**、`idf.py build` exit=0、bin 9,026,080 B、sdkconfig diff=0；报告 §16。L1b 屏接真实（seed/Start/Pause/Resume/Complete 门禁/重启恢复）→ L1c 上机（批次 ota_0 app-flash+monitor） |
