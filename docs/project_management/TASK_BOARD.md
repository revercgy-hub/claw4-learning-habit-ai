...
| 6.28 | CODEX-APP-FIRST-002 / 设备 Backend 真机联调 | Codex | `BLOCKED`（设备已进学习页；COM7 日志确认 TCP `0x71`，本机 relay 可监听但 Windows 私有网络入站规则创建需管理员权限；P1/P2/P3 `CHECKPOINT_READY`） | 6.27；AF3-5 刷写校验通过 | 解除条件：管理员 PowerShell 放行 `192.168.3.0/24 -> TCP 18765` 后复测 auth/today/events；不调度 WorkBuddy。 |
