...
| 6.28 | CODEX-APP-FIRST-002 / 设备 Backend 真机联调 | Codex | `IN_PROGRESS`（P1/P2 `CHECKPOINT_READY`；本机 relay 默认 `192.168.3.26:18765`；等待 Wi‑Fi/secret provisioning；不调度 WorkBuddy） | 6.27；AF3-5 刷写校验通过 | 有序 P0–P6：provisioning → HMAC signer → worker 调度 → 诊断 → Host/Backend 联调 → 一次 ota_0 真机验收；任务包 `CODEX-APP-FIRST-002_DEVICE_BACKEND_ON_DEVICE.md`。 |
| 6.29 | CODEX-APP-FIRST-002 / P1-P2 provisioning + HMAC signer | Codex | `CHECKPOINT_READY`（C5 BUILD ONLY；未刷本候选） | 6.28；用户确认 provisioning 方案 | 独立 `learning_cfg` NVS 配置、mbedTLS HMAC-SHA256/Base64 signer、`ConfigureProvisionedBackend()`；C5 `xiaozhi.bin` 9,179,872 B，SHA-256 `C57DF844…87872251`；报告 `CODEX_APP_FIRST_002_P1P2_2026-09-06.md`。 |
