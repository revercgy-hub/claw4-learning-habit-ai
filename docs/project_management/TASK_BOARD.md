...
L-03 根看板同步和 L-04 Preflight/正式 20 轮证据工具已完成。首个 S-05 对 R-02 Candidate 的 `ota_0` application-only 写入和独立读回通过，设备身份/live 布局及镜像 SHA 匹配；启动却实际连接外部 `api.tenclass.net` HTTPS/MQTT，语音刺激前停止，Preflight 为 **0 轮**，设备已由用户断电。S-04-ENDPOINT 已据此固定 M1 预飞行 NAS OTA/WS 端点、拒绝外部 MQTT/WS 和自动升级，冻结新 Candidate `claw4-learning-v6-m1-preflight-endpoint-s04-r03-20260925-03`；旧 app/ELF 为 `SUPERSEDED`。独立 R-03 复审 `PASS` 仅放行新 Candidate app-only 启动/端点核验；现场确认只访问 NAS 7443/7444 后才可进行 2～3 轮 Voice Preflight。DEVICE 与正式 20 轮仍未验证，M0 继续 `CHANGES_REQUIRED`。M1 Voice Core 保持单一 owner，不提前加入 Learning UI 或 M1.5 七命令 Router。详见 [V6_TASK_BOARD](../v6/V6_TASK_BOARD.md)、[S-05 设备报告](../v6/V6_M1_VOICE_PREFLIGHT_DEVICE_REPORT.md)和 [NAS 部署报告](../v6/V6_M1_NAS_DEPLOYMENT_REPORT.md)。
