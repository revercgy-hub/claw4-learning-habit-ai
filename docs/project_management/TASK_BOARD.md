...
| 6.28 | CODEX-APP-FIRST-002 / 设备 Backend 真机联调 | Codex | `READY`（等待 P0 endpoint/Wi-Fi/secret 注入方案；不调度 WorkBuddy） | 6.27；AF3-5 刷写校验通过 | 有序 P0–P6：provisioning → HMAC signer → worker 调度 → 诊断 → Host/Backend 联调 → 一次 ota_0 真机验收；任务包 `CODEX-APP-FIRST-002_DEVICE_BACKEND_ON_DEVICE.md`。 |
