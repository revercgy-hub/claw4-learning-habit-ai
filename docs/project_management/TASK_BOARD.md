...
| 6.28 | CODEX-APP-FIRST-002 / 设备 Backend 真机联调 | Codex | `IN_PROGRESS`（P1/P2/P3 `CHECKPOINT_READY`；NVS 已安全注入并回读；ota_0 已刷 worker 候选；等待用户进入学习页完成网络验收；不调度 WorkBuddy） | 6.27；AF3-5 刷写校验通过 | 有序 P0–P6：provisioning → HMAC signer → worker 调度 → 诊断 → Host/Backend 联调 → 一次 ota_0 真机验收；任务包 `CODEX-APP-FIRST-002_DEVICE_BACKEND_ON_DEVICE.md`。 |
