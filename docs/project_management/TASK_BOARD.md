...
| 6.23 | WB-LEARNING-V4-L1 Learning App 设备基本功能 | WorkBuddy | `IN_PROGRESS`（L1a 起） | `41f3646` | 任务包 `tasks/WB-LEARNING-V4_L1_APP_BASICS.md`：设备核心编入（host 业务核心镜像 E:/c/main/learning + manifest #3）、Device Ports（Clock=esp_timer / OutboxStorage=NVS ns learning）、序列化 codec（host roundtrip）、seed 演示快照、Learning 屏真实渲染、重启恢复；批次沿用 ota_0 app-flash+monitor；cp L1a codec+NVS+编入 build → L1b 屏接真实 → L1c 上机 |
