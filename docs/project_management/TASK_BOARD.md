...
| BLK-BUILD-STALE-001 | 冻结可信 L3 固件候选 | 6.31 只读回读证实：实机固件缺少 `pause_count` 字面量，根因是增量构建沿用 18:44 的陈旧 `reducer.cpp.obj`（镜像同步保留源文件旧 mtime，ninja 漏编）。当前实机 = `claw4-idf-cold-c5-20260906` 产物（9,255,536 B），6.29 记录的 `682FFBCB`（9,249,600 B）已不在本地 | **清洁重建**（`touch` 全部 learning 源文件或 clean build）→ 校验新 `.bin` 含 `pause_count` → 记录新 SHA-256 为唯一候选；随后经用户重新授权执行 ota_0 application-only 刷写 |
