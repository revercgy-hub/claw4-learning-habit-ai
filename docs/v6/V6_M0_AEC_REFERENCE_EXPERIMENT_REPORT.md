...
- A subsequent five-window no-touch UART observation spans app uptime `326743`–`627236` ms (300.493 s). All 30 health samples retained the same free-memory/PSRAM values and counters (`wake=1 taps=22 wakes=0 vads=6 local_playback_active=0`); 598 audio input samples reported `read_failures=0` and zero software-reference queue drops. The old `clipped=0` field is superseded: code review found its counter was never incremented after full-width normalization, so it provides no clipping evidence. No reset, boot, panic, abort, or `BOOT_BLOCKED` marker appeared. This is a short idle stability check only; it does not exercise playback, acoustic echo cancellation, or WakeNet with sound.
- The boot and idle windows had no touch, so local WakeNet during playback and AEC are not yet tested on Candidate17. Five private idle-capture SHA256 values, in order: `b0dbf4ab8ef8fe7eb9d58d7f57c2d10d6631856309323cc921e42c78c222514f`, `d3b0241a58f526a70dfe92c4fb04ead6e92e8c7e3eb08dbafed22a73fafdcdbe`, `99b2cbbea9d91dea6396eca49491b18dad3cc0b2195b06ff1699065b1ee11c90`, `83e1ba37f9234b5f8dbd15b0f29f4f18b0761b6aa5647d7c0f1024840faed68d`, `43ee746defec1755e8faba93a44fa1641a69b813c1f3db1ed470d48bf8770674`.
...
Other M0 gaps remain: device-level synchronous network failure injection, longer stability coverage beyond this five-minute idle window, physical verification of Candidate18's one-frame camera capture, and wake-word sensitivity/headroom. Candidate 13's sensor initialization and wake tests do not close those items. Candidate18's build-only camera implementation is documented in [`V6_M0_CANDIDATE18_CAMERA_REPORT.md`](V6_M0_CANDIDATE18_CAMERA_REPORT.md). Candidate19 adds a bounded PCM16 near-full-scale sample count for future input-level checks; this digital metric does not prove analog clipping. Raw UART captures remain in ignored `out/v6-device-private/`; no raw audio is stored in the repository.
